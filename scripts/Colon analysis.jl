...
out2 = pem_fit(state0, dat, priors, settings, test_times, burn_in)
println(out2[3]);println(out2[4])
priors = BasicPrior(1.0, PC(1.0, 2, 0.5, Inf), FixedW([0.5]), 1.0, CtsPois(7.0, 1.0, 100.0, 3.1), [GammaLangevin(2,7,1)], [0.1], 2)
out3 = pem_fit(state0, dat, priors, settings, test_times, burn_in)
println(out3[3]);println(out3[4])
priors = BasicPrior(1.0, PC(1.0, 2, 0.5, Inf), FixedW([0.5]), 1.0, CtsPois(7.0, 1.0, 100.0, 3.1), [GompertzBaseline(0.3)], [0.1], 2)
out4 = pem_fit(state0, dat, priors, settings, test_times, burn_in)
...
out5 = pem_fit(state0, dat, priors, settings, test_times, burn_in)
println(out5[3]);println(out5[4])
priors = BasicPrior(1.0, PC(1.0, 2, 0.5, Inf), FixedW([0.5]), 1.0, CtsNB(7.0, 1.0, 7.0, 1.0, 100.0, 3.1), [GaussLangevin(t -> log(0.29), t-> 0.4)], [0.1], 2)
out6 = pem_fit(state0, dat, priors, settings, test_times, burn_in)
println(out6[3]);println(out6[4])
priors = BasicPrior(1.0, PC(1.0, 2, 0.5, Inf), FixedW([0.5]), 1.0, CtsNB(7.0, 1.0, 7.0, 1.0, 100.0, 3.1), [GammaLangevin(2,7,1)], [0.1], 2)
out7 = pem_fit(state0, dat, priors, settings, test_times, burn_in)
println(out7[3]);println(out7[4])
priors = BasicPrior(1.0, PC(1.0, 2, 0.5, Inf), FixedW([0.5]), 1.0, CtsNB(7.0, 1.0, 7.0, 1.0, 100.0, 3.1), [GompertzBaseline(0.3)], [0.1], 2)
out8 = pem_fit(state0, dat, priors, settings, test_times, burn_in)
...
m_totalq1 = []
m_totalq2 = []
breaks_extrap = collect(3.12:0.02:10)
...
s1 = vcat(view(exp.(test_smp), 1, :, 5_000:10_000), view(exp.(extrap1), :, 5_000:10_000))
...
push!(m_obs, mean(get_meansurv(test_smp[1,:,5_000:10_000], grid, [1])))
push!(m_obsq1, quantile(get_meansurv(test_smp[1,:,5_000:10_000], grid, [1]), 0.025))
push!(m_obsq2, quantile(get_meansurv(test_smp[1,:,5_000:10_000], grid, [1]), 0.975))
