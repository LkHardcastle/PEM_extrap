...
priors1 = BasicPrior(1.0, PC([0.2], [2], [0.5], Inf), Beta([0.4], [1.0], [1.0]), 1.0, Cts(10.0, 50.0, 3.2), [RandomWalk()])
priors2 = BasicPrior(1.0, PC([0.2], [2], [0.5], Inf), Beta([0.4], [5.0], [5.0]), 1.0, Cts(10.0, 50.0, 3.2), [RandomWalk()])
priors3 = BasicPrior(1.0, PC([0.2], [2], [0.5], Inf), Beta([0.4], [10.0], [10.0]), 1.0, Cts(10.0, 50.0, 3.2), [RandomWalk()])
priors4 = BasicPrior(1.0, PC([0.2], [2], [0.5], Inf), Beta([0.4], [50.0], [50.0]), 1.0, Cts(10.0, 50.0, 3.2), [RandomWalk()])
Random.seed!(9102)
@time out1 = pem_sample(state0, dat, priors1, settings)
@time out2 = pem_sample(state0, dat, priors2, settings)
@time out3 = pem_sample(state0, dat, priors3, settings)
...
CSV.write(datadir("ColonSmps","Beta1.csv"), df5)
CSV.write(datadir("ColonSmps","Beta5.csv"), df6)
CSV.write(datadir("ColonSmps","Beta10.csv"), df7)
CSV.write(datadir("ColonSmps","Beta50.csv"), df8)
...
df5 = CSV.read(datadir("ColonSmps","Beta1.csv"), DataFrame)
df6 = CSV.read(datadir("ColonSmps","Beta5.csv"), DataFrame)
df7 = CSV.read(datadir("ColonSmps","Beta10.csv"), DataFrame)
df8 = CSV.read(datadir("ColonSmps","Beta50.csv"), DataFrame)

R"""
dat1 = data.frame($df1)
dat1 = cbind(dat1, "a = 0.1")
...
p3 <- dat_sigma %>%
    ...
    theme_classic() + guides(col = guide_legend(nrow = 2), linetype = FALSE) + 
    theme(legend.position = "bottom", text = element_text(size = 20)) + scale_colour_manual(values = cbPalette[c(8,4,6,7)]) +
    scale_linetype_manual(values = c("dotdash","solid","dotdash","dotdash","dotdash")) + ylab("h(t)") + xlab("Time (years)") + ylim(0,0.5)
p4 <- dat_beta %>%
    pivot_longer(Mean:UCI,) %>%
    ggplot(aes(x = Time, y = value, col = Model, linetype = name)) + geom_step() +
    theme_classic() + guides(col = guide_legend(nrow = 2), linetype = FALSE) + 
