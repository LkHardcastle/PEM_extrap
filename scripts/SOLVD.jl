...
priors1 = BasicPrior(1.0, PC([0.2, 0.2], [2, 2], [0.5, 0.5], Inf), Beta([0.4, 0.4], [10.0, 5.0], [10.0, 15.0]), 1.0, Cts(10.0, 150.0, 4.7), [RandomWalk(), GaussLangevin(-0.5,0.5)])
priors2 = BasicPrior(1.0, PC([0.2, 0.2], [2, 2], [0.5, 0.5], Inf), Beta([0.4, 0.4], [10.0, 5.0], [10.0, 15.0]), 1.0, Cts(10.0, 150.0, 4.7), [GaussLangevin(-1.0,1.0), GaussLangevin(-0.5,0.5)])
priors3 = BasicPrior(1.0, PC([0.2, 0.2], [2, 2], [0.5, 0.5], Inf), Beta([0.4, 0.4], [10.0, 5.0], [10.0, 15.0]), 1.0, Cts(10.0, 150.0, 4.7), [GammaLangevin(0.5,2), GaussLangevin(-0.5,0.5)])
priors4 = BasicPrior(1.0, PC([0.2, 0.2], [2, 2], [0.5, 0.5], Inf), Beta([0.4, 0.4], [10.0, 5.0], [10.0, 15.0]), 1.0, Cts(10.0, 150.0, 4.7), [GompertzBaseline(0.5), GaussLangevin(-0.5,0.5)])
Random.seed!(9102)
@time out1 = pem_sample(state0, dat, priors1, settings)
@time out2 = pem_sample(state0, dat, priors2, settings)
@time out3 = pem_sample(state0, dat, priors3, settings)
...
df1 = DataFrame(hcat(vcat(grid, breaks_extrap), median(s1, dims = 2), quantile.(eachrow(s1), 0.025),  quantile.(eachrow(s1), 0.975), median(s2, dims = 2), quantile.(eachrow(s2), 0.025),  quantile.(eachrow(s2), 0.975)), :auto)
...
s1 = vcat(view(exp.(test_smp), 1, :, :), view(exp.(extrap1), :, :))
s2 = vcat(exp.(test_smp[1,:,:] .+ test_smp[2,:,:]), exp.(extrap1 .+ extrap2))
df2 = DataFrame(hcat(vcat(grid, breaks_extrap), median(s1, dims = 2), quantile.(eachrow(s1), 0.025),  quantile.(eachrow(s1), 0.975), median(s2, dims = 2), quantile.(eachrow(s2), 0.025),  quantile.(eachrow(s2), 0.975)), :auto)
...
s1 = vcat(view(exp.(test_smp), 1, :, :), view(exp.(extrap1), :, :))
s2 = vcat(exp.(test_smp[1,:,:] .+ test_smp[2,:,:]), exp.(extrap1 .+ extrap2))
df3 = DataFrame(hcat(vcat(grid, breaks_extrap), median(s1, dims = 2), quantile.(eachrow(s1), 0.025),  quantile.(eachrow(s1), 0.975), median(s2, dims = 2), quantile.(eachrow(s2), 0.025),  quantile.(eachrow(s2), 0.975)), :auto)
...
p1 <- dat1 %>%
    ...
    scale_linetype_manual(values = c("dotdash","dotdash","solid", "solid", "dotdash","dotdash")) + ylab("h(t)") + xlab("Time (years)") + ylim(0,1) + xlim(0,4.6)
p2 <- dat2 %>%
    pivot_longer(Median1:UCI2) %>%
    ggplot(aes(x = Time, y = value, col = name,linetype = name)) + geom_step() +
    theme_classic() +
...
p1 <- dat1 %>%
    ...
    geom_vline(xintercept = 4.6) +
    theme(legend.position = "none",text = element_text(size = 20)) + scale_colour_manual(values = cbPalette[c(6,7,6,7,6,7)]) +
    scale_linetype_manual(values = c("dotdash","dotdash","solid", "solid", "dotdash","dotdash")) + ylab("h(t)") + xlab("Time (years)") + ylim(0,1) + xlim(0,15)
p2 <- dat2 %>%
    pivot_longer(Median1:UCI2) %>%
    ggplot(aes(x = Time, y = value, col = name,linetype = name)) + geom_step() +
    theme_classic() +
...
plot(cumsum(out1["Smp_x"][2,:,1_000], dims = 1))
ind = 100:1_800
plot(out1["Smp_s_loc"][:,ind], cumsum(out1["Smp_x"][2,:,ind], dims = 1),linetype=:steppost, legend = false, alpha = 0.05, color = "black")

anim = Animation()
for ind in 100:1_800
    plot(out1["Smp_s_loc"][:,ind], exp.(cumsum(out1["Smp_x"][1,:,ind], dims = 1)),linetype=:steppost, legend = false, color = "black", ylim = (0,1))
    frame(anim)
end
gif(anim, "lol.gif", fps=5)

anim = Animation()
for ind in 100:1_800
    plot(out1["Smp_s_loc"][:,ind], cumsum(out1["Smp_x"][2,:,ind], dims = 1),linetype=:steppost, legend = false, color = "black", ylim = (-1.5, 0.5))
    frame(anim)
end
gif(anim, "lol.gif", fps=5)