...
settings = Settings(100,0.5)
Random.seed!(123)
out = @time pem_sample(x0, s0, v0, t0, dat, priors, settings)

plot(out["t"][1:100], vec(out["Sk_x"][:,1,:])[1:100])
plot!(out["t"][1:100], vec(out["Sk_x"][:,2,:])[1:100])
...
plot(vec(sum(out["Sk_s"], dims = 2)))

smps = post_estimates(out, dat, collect(1:0.5:2200))

smps
plot(eachcol(smps), layout = 8)
exp.(median(smps, dims = 1))
exp.(quantile.(eachcol(smps), 0.05))
exp.(quantile.(eachcol(smps), 0.95))
out["Eval"]