...
settings = Settings(nits, nsmp, 100000, 0.5,0.0, 0.2, false, true)
Random.seed!(123)
priors = BasicPrior(1.0, FixedV(0.1), FixedW(0.5), 1.0)
@time out1 = pem_sample(state0, dat, priors, settings)
...
nits = 100_000
nsmp = 100000
settings = Settings(nits, nsmp, 100000, 0.5,1.0, 0.2, false, true)
Random.seed!(123)
priors = BasicPrior(1.0, FixedV(0.5), Beta(0.2, 5.0, 15.0), 1.0)
@time out4 = pem_sample(state0, dat, priors, settings)
priors = BasicPrior(1.0, PC(0.5, 1, 0.1, 1, Inf), Beta(0.4, 10.0, 10.0), 1.0)
@time out5 = pem_sample(state0, dat, priors, settings)
priors = BasicPrior(1.0, FixedV(0.5), Beta(0.6, 15.0, 5.0), 1.0)
@time out6 = pem_sample(state0, dat, priors, settings)

plot(vec(out4["Smp_h"][2,:]))
plot!(vec(out5["Smp_h"][2,:]))
plot!(vec(out6["Smp_h"][2,:]))
smps1 = out4["Smp_trans"]
s1 = view(smps1, 1, :, :)
plot(vcat(0,breaks), vcat(mean(exp.(s1), dims = 2), mean(exp.(s1), dims = 2)[end]),linetype=:steppost)
plot!(vcat(0,breaks),vcat(quantile.(eachrow(exp.(s1)), 0.025),quantile.(eachrow(exp.(s1)), 0.025)[end]),linetype=:steppost)
...
x_plot = out1["Sk_x"][:,1:31,1:10000]
plot(x_plot[1,1,:], x_plot[1,2,:])
plot(out1["Sk_t"][1:5000],x_plot[1,28,1:5000])

plot(vec(out1["Smp_h"][1,:]))
plot(vec(out2["Smp_h"][1,:]))
plot(vec(out3["Smp_h"][1,:]))
...
nits = 100_000
nsmp = 100000
settings = Settings(nits, nsmp, 100000, 0.5,1.0, 0.1, false, true)
Random.seed!(123)
priors = BasicPrior(1.0, PC(0.2, 2, 0.1, 1, Inf), Beta(0.4, 4.0, 6.0), 1.0)
@time out1 = pem_sample(state0, dat, priors, settings)
@time out11 = pem_sample(state0, dat, priors, settings)
...
t0 = 0.0
state0 = ECMC2(x0, v0, s0, t0, true, findall(s0))
priors = BasicPrior(1.0, PC(0.2, 2, 0.1, 1, Inf), Beta(0.2, 2.0, 8.0), 1.0)
@time out2 = pem_sample(state0, dat, priors, settings)
@time out21 = pem_sample(state0, dat, priors, settings)
...
t0 = 0.0
state0 = ECMC2(x0, v0, s0, t0, true, findall(s0))
priors = BasicPrior(1.0, PC(0.2, 2, 0.1, 1, Inf), Beta(0.1, 1, 9.0), 1.0)
@time out3 = pem_sample(state0, dat, priors, settings)
@time out31 = pem_sample(state0, dat, priors, settings)
...
breaks = collect(0.2:0.2:3.2)
s1 = view(smps1, 1, :, :)
plot(vcat(0,breaks), vcat(mean(exp.(s1), dims = 2), mean(exp.(s1), dims = 2)[end]),linetype=:steppost, color = :blue)
plot!(vcat(0,breaks),vcat(quantile.(eachrow(exp.(s1)), 0.025),quantile.(eachrow(exp.(s1)), 0.025)[end]),linetype=:steppost, color = :blue)
plot!(vcat(0,breaks),vcat(quantile.(eachrow(exp.(s1)), 0.975),quantile.(eachrow(exp.(s1)), 0.975)[end]),linetype=:steppost, ylim = (0,.6), color = :blue)
smps1 = out2["Smp_trans"]
breaks = collect(0.1:0.1:3.1)
s1 = view(smps1, 1, :, :)
plot!(vcat(0,breaks), vcat(mean(exp.(s1), dims = 2), mean(exp.(s1), dims = 2)[end]),linetype=:steppost, color = :green)
plot!(vcat(0,breaks),vcat(quantile.(eachrow(exp.(s1)), 0.025),quantile.(eachrow(exp.(s1)), 0.025)[end]),linetype=:steppost, color = :green)
plot!(vcat(0,breaks),vcat(quantile.(eachrow(exp.(s1)), 0.975),quantile.(eachrow(exp.(s1)), 0.975)[end]),linetype=:steppost, ylim = (0,.6), color = :green)
smps1 = out3["Smp_trans"]
breaks = collect(0.05:0.05:3.05)
s1 = view(smps1, 1, :, :)
plot!(vcat(0,breaks), vcat(mean(exp.(s1), dims = 2), mean(exp.(s1), dims = 2)[end]),linetype=:steppost, color = :red)
plot!(vcat(0,breaks),vcat(quantile.(eachrow(exp.(s1)), 0.025),quantile.(eachrow(exp.(s1)), 0.025)[end]),linetype=:steppost, color = :red)
plot!(vcat(0,breaks),vcat(quantile.(eachrow(exp.(s1)), 0.975),quantile.(eachrow(exp.(s1)), 0.975)[end]),linetype=:steppost, ylim = (0,.6), color = :red)