function ∇U(x::Matrix{Float64}, v::Matrix{Float64}, s::Matrix{Bool}, dat::PEMData, j::CartesianIndex, priors::Prior)
    ∇Uλ = 0.0
    Uλ = 0.0
    ∇U_out = []
...
function ∇U_bound!(x::Matrix{Float64}, v::Matrix{Float64}, s::Matrix{Bool}, dat::PEMData, priors::Prior, j::CartesianIndex, dyn::Dynamics)
    ∇U1 = ∇U(x, s, dat, CartesianIndex(1,1), priors)
    ∇U2 = ∇U(x .+ v.*dyn.next_bound_int, s, dat, CartesianIndex(1,1), priors)
    m = 1
    dyn.a = 0.0
    dyn.b = 0.0
    for k ∈ CartesianIndex(1,1):CartesianIndex(size(s,1),size(s,2))
        if s[k]
            if v[k] > 0.0
                ...
                dyn.a += ΛU1 #+ 0.01 
                dyn.b += (ΛU2 - ΛU1)/dyn.next_bound_int
            elseif v[j] < 0.0
                ΛU1 = max(v[k]*∇U1[m], 0.0)
                ΛU2 = max(v[k]*∇U2[m], 0.0)
