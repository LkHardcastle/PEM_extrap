...
function pem_sample(x0::Matrix{Float64}, s0::Matrix{Bool}, v0::Matrix{Float64}, t0::Float64, dat::PEMData, priors::Prior, settings::Settings, dyn::Dynamics)
    ...
    while dyn.ind < settings.max_ind
        ...
        x, v, s, t = sampler_inner!(x, s, v, t, dat, priors, dyn, times)
...
function sampler_inner!(x::Matrix{Float64}, s::Matrix{Bool}, v::Matrix{Float64}, t::Float64, dat::PEMData, priors::Prior, dyn::Dynamics, times::Times)
    inner_stop = false
    while !inner_stop
        ...
        if dyn.next_event_type == 1
            if dyn.new_bound
                ...
                inner_stop = flip_attempt!(x, v, s, t, dat, times, dyn, priors)
            end
